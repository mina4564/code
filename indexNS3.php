...
// 引入 mPDF 的 autoload 文件
require_once('/home/chchting/vendor/autoload.php');
// 檢查是否有生成 PDF 的請求
...
<script>
    ...
    document.getElementById('print').addEventListener('click', function () {
        console.log('print'); // 輸出到控制台以確認按鈕被點擊
    // 顯示一個提示用戶正在生成 PDF
    var button = document.getElementById('print');  // 取得「print」按鈕元素
    button.disabled = true;  // 禁用按鈕，防止用戶再次點擊
    button.innerHTML = '正在生成 PDF，請稍候...';  // 修改按鈕文字，提示用戶操作正在進行中

    // 建立一個 XMLHttpRequest 物件，向伺服器發送請求以生成 PDF
    var xhr = new XMLHttpRequest();
    xhr.open('GET', '?action=generate_pdf', true);  // 使用 GET 請求，並且帶上 action=generate_pdf 的 URL 參數
    xhr.responseType = 'blob';  // 指定請求返回的是二進制數據（PDF 文件）

    // 當請求成功響應後觸發 onload 函數
    xhr.onload = function () {
        if (this.status === 200) {  // 確認請求成功，HTTP 狀態碼為 200
            var blob = new Blob([this.response], { type: 'application/pdf' });  // 將響應數據轉換為 PDF Blob 物件
            var link = document.createElement('a');  // 創建一個 <a> 鏈接元素
            link.href = window.URL.createObjectURL(blob);  // 將 Blob 轉換為一個可下載的 URL
            link.download = 'report.pdf';  // 設定下載的文件名為 report.pdf
            link.click();  // 模擬點擊 <a> 元素，觸發下載

            // 下載完成後，還原按鈕狀態
            button.disabled = false;  // 啟用按鈕
            button.innerHTML = 'Download PDF';  // 恢復按鈕的文字
        }
    };
...
<?php
 $html = ob_get_clean();  // 獲取輸出的內容並清除緩衝區
 $mpdf->WriteHTML($html);
 ob_end_clean();  // 清除緩衝，防止額外輸出
    // 設定正確的 HTTP header 並輸出 PDF
    header('Content-Type: application/pdf');
    header('Content-Disposition: attachment; filename="report.pdf"');
    $mpdf->Output(); // 生成 PDF 並輸出
    exit; // 結束程式，避免後續 HTML 繼續輸出
}

// 如果不是 PDF 請求，顯示正常的 HTML 頁面
?>
